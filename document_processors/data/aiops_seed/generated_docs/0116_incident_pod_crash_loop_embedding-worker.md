# embedding-worker - KubernetesPodCrashLooping 故障案例

## 元数据

- 文档ID: `AIOPS-INCIDENT-0116-pod_crash_loop-embedding-worker`
...
`embedding-worker` 触发 `KubernetesPodCrashLooping`，触发条件为：Pod 进入 CrashLoopBackOff 且 5 分钟内重启超过 3 次。
