# vector-indexer - KubernetesPodCrashLooping 故障案例

## 元数据

- 文档ID: `AIOPS-INCIDENT-0114-pod_crash_loop-vector-indexer`
...
`vector-indexer` 触发 `KubernetesPodCrashLooping`，触发条件为：Pod 进入 CrashLoopBackOff 且 5 分钟内重启超过 3 次。
