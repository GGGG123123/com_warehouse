...
from .utils import build_retry_update, clear_retry_update
...
async def executor(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    if not plan:
        ...
        return clear_retry_update(state, NODE_NAME)
...
- 专注于当前步骤，不要考虑其他任务"""),
        ...
        if retry_update.get("retrying_node") == NODE_NAME:
            logger.warning(
                f"执行步骤失败，保留当前步骤并重试: {task}, "
                f"retry={retry_update.get('retry_attempt')}, error={e}",
                exc_info=True,
            )
            return retry_update

        logger.error(f"执行步骤失败，重试耗尽: {task}, 错误: {e}", exc_info=True)
        return {
            **clear_retry_update(state, NODE_NAME),
            "plan": plan[1:],
