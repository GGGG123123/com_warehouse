...
from .utils import build_retry_update, clear_retry_update, format_tools_description
...
async def replanner(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    if len(past_steps) >= MAX_STEPS:
        ...
        response_update = await _generate_response(state, llm)
        return {
            **clear_retry_update(state, NODE_NAME),
            **response_update,
        }
    ...
    if plan:
        ...
        try:
            ...
            retry_update = build_retry_update(state, NODE_NAME, e)
            if retry_update.get("retrying_node") == NODE_NAME:
                logger.warning(
                    f"重新规划失败，准备重试 Replanner: "
                    f"retry={retry_update.get('retry_attempt')}, error={e}",
                    exc_info=True,
                )
                return retry_update

            logger.error(f"重新规划失败，重试耗尽，继续执行剩余计划: {e}", exc_info=True)
            return {
                **clear_retry_update(state, NODE_NAME),
                "plan": plan,
