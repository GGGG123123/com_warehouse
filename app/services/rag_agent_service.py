...
class ShortTermMemoryTrimMiddleware(AgentMiddleware):
    """在每次模型调用前裁剪短期消息，避免上下文窗口无限增长。"""

    def before_model(self, state: AgentState, runtime: Any) -> dict[str, Any] | None:
        return trim_messages_middleware(state)
...
class RagAgentService:
    ...
    async def _initialize_agent(self):
        ...
        self.agent = create_agent(
            self.model,
            tools=all_tools,
            middleware=(ShortTermMemoryTrimMiddleware(),),
    ...
    async def query(
        ...
        try:
            ...
            messages = [
                SystemMessage(content=runtime_system_prompt),
    ...
    def _build_runtime_system_prompt(
        ...
        """为本次调用拼接结构化用户记忆。"""
        memory_context = structured_memory_service.build_user_memory_context(
            tenant_id=tenant_id,
            user_id=user_id,
            session_id=session_id,
        )
