...
class SuperBizAgentApp {
    constructor() {
        this.apiBaseUrl = window.location.origin + '/api';  // 自动使用当前访问的域名和端口
        this.currentMode = 'quick'; // 'quick' 或 'stream'
        this.sessionId = this.generateSessionId();
        this.tenantId = 'local';
        this.userId = this.getOrCreateUserId();
        this.isStreaming = false;
        this.pendingAttachments = []; // 本轮多模态输入附件，不入知识库
    ...
    // 获取或创建当前浏览器用户 ID，用于结构化偏好记忆隔离
    getOrCreateUserId() {
        const storageKey = 'superBizAgentUserId';
        let userId = localStorage.getItem(storageKey);
        if (!userId) {
            userId = `browser_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    ...
    initializeElements() {
        ...
        this.attachInputItem = document.getElementById('attachInputItem');
        ...
        this.attachmentPreviewList = document.getElementById('attachmentPreviewList');
    ...
    bindEvents() {
        ...
        if (this.attachInputItem) {
            this.attachInputItem.addEventListener('click', () => {
        ...
        if (this.attachmentInput) {
            this.attachmentInput.addEventListener('change', (e) => this.handleAttachmentSelect(e));
        }
    ...
    async sendMessage() {
        ...
        if (!message && attachments.length === 0) {
            this.showNotification('请输入消息内容或添加附件', 'warning');
            return;
        }
        ...
        // 显示用户消息
        this.addMessage('user', this.buildUserDisplayMessage(message, attachments));
        ...
        try {
            if (attachments.length > 0) {
                if (this.currentMode === 'stream') {
                    this.showNotification('附件输入使用快速模式处理', 'info');
                }
                await this.sendMultimodalMessage(message, attachments);
    ...
    // 处理多模态附件选择：这些附件只用于本轮对话，不会上传到知识库
    ...
    handleInputPaste(event) {
        ...
        const files = items
            ...
            .map(file => {
                ...
                return new File([file], `screenshot_${Date.now()}.${extension}`, {
    ...
    // 添加待发送附件，并进行前端类型和大小校验
    addPendingAttachments(files) {
        const maxCount = 6;
        for (const file of files) {
            if (this.pendingAttachments.length >= maxCount) {
                this.showNotification(`一次最多添加 ${maxCount} 个附件`, 'warning');
                break;
            }
    ...
    validateAttachment(file) {
        ...
        if (file.size > maxSize) {
            return {
                valid: false,
                message: `${file.name} 超过10MB，不能作为附件输入`
            };
        }
        ...
        const allowedTextExtensions = [
            ...
            '.yaml', '.yml', '.xml', '.html', '.css', '.js', '.ts', '.py',
            '.sql', '.ini', '.conf'
        ];
        const isImage = file.type && file.type.startsWith('image/');
        const isTextFile = allowedTextExtensions.some(ext => fileName.endsWith(ext));
        const isTextMime = file.type && file.type.startsWith('text/');

        if (!isImage && !isTextFile && !isTextMime) {
            return {
                valid: false,
                message: `${file.name} 暂不支持，请使用图片、截图或文本类文件`
            };
        }
    ...
    renderAttachmentPreviews() {
        if (!this.attachmentPreviewList) {
            return;
        }

        this.attachmentPreviewList.innerHTML = '';
        if (this.pendingAttachments.length === 0) {
            this.attachmentPreviewList.classList.remove('visible');
            return;
        }

        this.attachmentPreviewList.classList.add('visible');
        this.pendingAttachments.forEach(attachment => {
            ...
            icon.innerHTML = file.type && file.type.startsWith('image/')
                ? `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="3" y="5" width="18" height="14" rx="2" stroke="currentColor" stroke-width="2"/><path d="M8 13L10.5 10.5L14 14L15.5 12.5L19 16" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>`
                : `<svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8Z" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/><path d="M14 2v6h6" stroke="currentColor" stroke-width="2" stroke-linejoin="round"/></svg>`;

            const text = document.createElement('span');
            text.className = 'attachment-chip-text';
            text.textContent = `${file.name} · ${this.formatFileSize(file.size)}`;
            ...
            `;
            removeButton.addEventListener('click', () => this.removePendingAttachment(attachment.id));

            chip.appendChild(icon);
            chip.appendChild(text);
            chip.appendChild(removeButton);
            this.attachmentPreviewList.appendChild(chip);
        });
    }

    // 删除某个待发送附件
    removePendingAttachment(attachmentId) {
        this.pendingAttachments = this.pendingAttachments.filter(item => item.id !== attachmentId);
        this.renderAttachmentPreviews();
    }
    ...
    // 用户气泡里展示附件名称，避免把二进制内容保存到浏览器历史
    buildUserDisplayMessage(message, attachments) {
        const lines = [];
        if (message) {
            lines.push(message);
        }
    ...
    async sendMultimodalMessage(message, attachments) {
        const loadingMessage = this.addLoadingMessage('正在理解附件...');
        ...
        try {
            ...
            const response = await fetch(`${this.apiBaseUrl}/chat_multimodal`, {
                method: 'POST',
                body: formData
            });
            ...
            if (data.code === 200 && data.data && data.data.success) {
                this.addMessage('assistant', data.data.answer || '（无回复内容）');
                return;
            }

            throw new Error(data.data?.errorMessage || data.message || '多模态请求失败');
        } catch (error) {
            if (loadingMessage && loadingMessage.parentNode) {
                loadingMessage.parentNode.removeChild(loadingMessage);
            }
