...
| 系统状态 | GET | `/api/monitoring/status` | 实时系统资源监控 |
| 错误日志 | GET | `/api/monitoring/errors` | 获取最近错误日志 |
...
**健康检查监控（推荐）：**
```powershell
.\health_check.ps1  # 每分钟检查服务状态，异常时告警
```

**实时监控面板：**
```powershell
.\monitor.ps1  # 实时显示系统状态、资源占用、错误日志
```
...
**1. 获取系统状态：**
```powershell
Invoke-RestMethod -Uri "http://localhost:9900/api/monitoring/status"
```

**2. 获取错误日志：**
```powershell
Invoke-RestMethod -Uri "http://localhost:9900/api/monitoring/errors"
```

**3. 实时查看日志文件：**
```powershell
Get-Content logs\app_$(Get-Date -Format "yyyy-MM-dd").log -Wait -Tail 50
```

**详细监控方案请查看：** [MONITORING.md](MONITORING.md)
